...
const getById = async (id: string) =>
  await CustomerModel.findById(id).then(
    (val) => val ?? Promise.reject("Customer not found")
  )

const getAll = async () =>
  await CustomerModel.find().then(
...
const deleteById = async (id: string) =>
  await CustomerModel.findByIdAndDelete(id).then(
    (val) => val ?? Promise.reject("Customer not found")
  )
